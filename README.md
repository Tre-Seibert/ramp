...
Fix:
In the index.css file the .RampInputSelect--dropdown-container is a fixed position. We can fix this bug by making it deleting
this line and making it not fixed.
...
Fix:
Inside index.tsx in the InputCheckbox folder, the label element didn't have a htmlFor attribute that links it to the input,
making the functionality non existent. By adding a simple htmlFor attribute the bug is fixed in all cases.
